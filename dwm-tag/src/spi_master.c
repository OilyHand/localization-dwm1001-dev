...
LOG_MODULE_REGISTER(spi_master);
...
void spi_master_init(void)
{
    ...
    LOG_INF("SPI Master example application\n");

    LOG_INF("rx_data buffer at %p\n", &rx_data);
    ...
    while (1) {
        ...
        LOG_INF("Received: 0x%04X\n", rx_data);

        LOG_INF("Response is %s\n",  (rx_data == 0x5678) ? "ok" : "wrong");
